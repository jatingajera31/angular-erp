...
@Component({
  selector: 'app-demo-return-receipt',
  templateUrl: './demo-return-receipt.component.html',
  styleUrls: ['./demo-return-receipt.component.css']
})
export class DemoReturnReceiptComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    this.purchaseForm = this.fb.group({
      ...
      demo_receipt_no: new FormControl(null, [Validators.required]),
      project_id: new FormControl(null),
      demo_dispatch_id: new FormControl(null, [Validators.required, Validators.pattern("^[0-9]*$")]),
      ...
      demo_receipt_date: new FormControl(null, [Validators.required]),
      demo_receipt_time: new FormControl(null, [Validators.required]),
      remarks: new FormControl(null),
      t_demo_receipt_date: new FormControl(null),
      demo_dispatch_no: new FormControl(null),
      demo_dispatch_date: new FormControl(null),
  ...
  getDispatched() {
    this.loader.start();
    this.apiService.getDemoDispatch({is_return: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
        this.dispatchedDemands = data['data'];
        this.dispatchedDemands.forEach((item) => {
          item.delayed = false;
          let date1: any = new Date(item.demo_dispatch_date);
          let date2: any = new Date();
          let diffTime = Math.abs(date2 - date1);
          item.diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
          if (item.demo) {
            date1.setDate(date1.getDate()+Number(item.demo.demo_days));
  ...
  getDemoReceiptNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getDemoReceiptNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.purchaseForm.controls.demo_receipt_no.setValue(data['data']);
        }
        this.loader.stop();
      });
    }
  ...
  setRow(item: any) {
    ...
    this.productDetails = [];
    this.setInitDate();
    this.purchaseForm.controls.demo_dispatch_id.setValue(item.id);
    ...
    this.purchaseForm.controls.demo_dispatch_no.setValue(item.demo_dispatch_no);
    this.purchaseForm.controls.demo_dispatch_date.setValue(item.demo_dispatch_date);
    this.getDemoReceiptNo();
  ...
  showDemoDispatch() {
    this.loader.start();
    this.apiService.showDemoDispatch({id: this.purchaseForm.value.demo_dispatch_id}).subscribe(data => {
      this.loader.stop();
  ...
  setInitDate() {
    ...
    this.purchaseForm.controls.demo_receipt_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.demo_receipt_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateDemoReceipt(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Demo Return Updated Successfully');
          this.closeForm();
      ...
      this.loader.start();
      this.apiService.saveDemoReceipt(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Demo Return Saved Successfully');
          this.closeForm();
          this.apiService.applyNotificationCount('Demo');
        }
        if (data['status'] == 0) {
  ...
  showDemoReceipt() {
    this.loader.start();
    this.apiService.showDemoReceipt({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1 && data['data']) {
  ...
  getDemoReceipt() {
    ...
    this.loader.start();
    this.apiService.getDemoReceipt({client_id: this.clientId}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  deleteInfo() {
    if (this.purchaseForm.value.id && this.purchaseForm.value.id > 0) {
      ...
      this.toastr.error('ERROR', 'Please Select Demo Return.');
    }
  }

  deleteData() {
    if (!this.purchaseForm.value.id) {
      this.toastr.error('ERROR', 'Please Select One Demo Return.');
    } else {
      this.loader.start();
      this.apiService.deleteService({id: this.purchaseForm.value.id}).subscribe(data => {
        ...
        if (data && data['status'] == 1) {
          ...
          this.toastr.success('SUCCESS', 'Demo Return deleted successfully.'); 
        }
      });
    }
  }
