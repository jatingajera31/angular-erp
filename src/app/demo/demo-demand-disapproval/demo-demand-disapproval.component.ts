...
@Component({
  selector: 'app-demo-demand-disapproval',
  templateUrl: './demo-demand-disapproval.component.html',
  styleUrls: ['./demo-demand-disapproval.component.css']
})
export class DemoDemandDisapprovalComponent implements OnInit {
  ...
  getEditClients() {
    this.apiService.editClients({page: 'dd', status: 'Approved', disapprove: 1}).subscribe(data => {
  ...
  getDemoDemand(status: string) {
    this.loader.start();
    this.apiService.getDemoDemand({status: status, client_id: this.clientId, disapprove: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  showDemoDemand() {
    this.loader.start();
    this.apiService.showDemoDemand({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.demoDemandApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Demo Demand Disapproved Successfully.');
          this.closeForm();
