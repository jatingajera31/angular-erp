...
@Component({
  selector: 'app-demo-demand-approval',
  templateUrl: './demo-demand-approval.component.html',
  styleUrls: ['./demo-demand-approval.component.css']
})
export class DemoDemandApprovalComponent implements OnInit {
  ...
  getDemoDemand(status: string) {
    this.loader.start();
    this.apiService.getDemoDemand({status: status}).subscribe(data => {
      this.loader.stop();
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      ...
      this.apiService.demoDemandApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Demo Demand Approved Successfully.');
          this.closeForm();
