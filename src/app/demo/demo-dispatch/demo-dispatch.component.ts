...
@Component({
  selector: 'app-demo-dispatch',
  templateUrl: './demo-dispatch.component.html',
  styleUrls: ['./demo-dispatch.component.css']
})
export class DemoDispatchComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    this.purchaseForm = this.fb.group({
      ...
      demo_id: new FormControl(null, [Validators.required, Validators.pattern("^[0-9]*$")]),
      ...
      demo_dispatch_date: new FormControl(null, [Validators.required]),
      demo_dispatch_time: new FormControl(null, [Validators.required]),
      demo_dispatch_no: new FormControl(null, [Validators.required]),
      t_demo_dispatch_date: new FormControl(null),
      project_id: new FormControl(null),
      client: new FormControl(null),
      location: new FormControl(null),
      project: new FormControl(null),
      demo_no: new FormControl(null),
      demo_date: new FormControl(null),
      demo_time: new FormControl(null),
  ...
  getDemoDemand(status: string) {
    this.loader.start();
    this.apiService.getDemoDemand({status: status, dispatch: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  getDispatched() {
    ...
    this.loader.start();
    this.apiService.getDemoDispatch({client_id: this.clientId}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  getDemoDispatchNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getDemoDispatchNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.purchaseForm.controls.demo_dispatch_no.setValue(data['data']);
        }
        this.loader.stop();
      });
    }
  ...
  setRow(item: any) {
    this.setInitDate();
    this.purchaseForm.controls.demo_id.setValue(item.id);
    ...
    this.purchaseForm.controls.demo_no.setValue(item.demo_no);
    this.purchaseForm.controls.demo_date.setValue(item.demo_date);
    this.purchaseForm.controls.demo_time.setValue(item.demo_time);
  ...
  showDemoDemand() {
    this.loader.start();
    this.apiService.showDemoDemand({id: this.purchaseForm.value.demo_id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  showDemoDispatch() {
    this.loader.start();
    this.apiService.showDemoDispatch({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1 && data['data']) {
        ...
        if (data['demo']) {
          let project = (data['demo'].project) ? data['demo'].project.name: '';
          this.purchaseForm.controls.client.setValue(data['demo'].client.account_name);
          this.purchaseForm.controls.location.setValue(data['demo'].location.name);
          this.purchaseForm.controls.project.setValue(project);
          this.purchaseForm.controls.demo_no.setValue(data['demo'].demo_no);
          this.purchaseForm.controls.demo_date.setValue(data['demo'].demo_date);
          this.purchaseForm.controls.demo_time.setValue(data['demo'].demo_time);
          if (data['demo'].statusby) {
            let n = data['demo'].statusby.first_name +' '+data['demo'].statusby.father_name +' '+data['demo'].statusby.last_name;
            this.purchaseForm.controls.approved_by.setValue(n);
            this.purchaseForm.controls.status_date.setValue(data['demo'].status_date);
            this.purchaseForm.controls.status_time.setValue(data['demo'].status_time);
          }
        }
        this.showDemoDemand()
      }
  ...
  viewCreateMode() {
    ...
    this.editMode = false;
    this.purchaseForm.reset();
    this.getDemoDemand('Approved');
  }

  setInitDate() {
    const date = this.datePipe.transform(new Date(), 'yyyy-MM-dd');
    this.purchaseForm.controls.demo_dispatch_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.demo_dispatch_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateDemoDispatch(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
      ...
      this.loader.start();
      this.apiService.saveDemoDispatch(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Dispatch Detail Changed Successfully');
          this.closeForm();
