...
@Component({
  selector: 'app-sample-return-receipt',
  templateUrl: './sample-return-receipt.component.html',
  styleUrls: ['./sample-return-receipt.component.css']
})
export class SampleReturnReceiptComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    this.purchaseForm = this.fb.group({
      ...
      sample_receipt_no: new FormControl(null, [Validators.required]),
      project_id: new FormControl(null),
      sample_dispatch_id: new FormControl(null, [Validators.required, Validators.pattern("^[0-9]*$")]),
      ...
      sample_receipt_date: new FormControl(null, [Validators.required]),
      sample_receipt_time: new FormControl(null, [Validators.required]),
      remarks: new FormControl(null),
      t_sample_receipt_date: new FormControl(null),
      sample_dispatch_no: new FormControl(null),
      sample_dispatch_date: new FormControl(null),
  ...
  getDispatched() {
    this.loader.start();
    this.apiService.getSampleDispatch({is_return: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
        this.dispatchedDemands = data['data'];
        this.dispatchedDemands.forEach((item) => {
          item.delayed = false;
          let date1: any = new Date(item.sample_dispatch_date);
          let date2: any = new Date();
          let diffTime = Math.abs(date2 - date1);
          item.diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
          if (item.sample) {
            date1.setDate(date1.getDate()+Number(item.sample.sample_days));
  ...
  getSampleReceiptNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getSampleReceiptNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.purchaseForm.controls.sample_receipt_no.setValue(data['data']);
        }
        this.loader.stop();
      });
    }
  ...
  setRow(item: any) {
    ...
    this.productDetails = [];
    this.setInitDate();
    this.purchaseForm.controls.sample_dispatch_id.setValue(item.id);
    ...
    this.purchaseForm.controls.sample_dispatch_no.setValue(item.sample_dispatch_no);
    this.purchaseForm.controls.sample_dispatch_date.setValue(item.sample_dispatch_date);
    this.getSampleReceiptNo();
  ...
  showSampleDispatch() {
    this.loader.start();
    this.apiService.showSampleDispatch({id: this.purchaseForm.value.sample_dispatch_id}).subscribe(data => {
      this.loader.stop();
  ...
  setInitDate() {
    ...
    this.purchaseForm.controls.sample_receipt_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.sample_receipt_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateSampleReceipt(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sample Return Updated Successfully');
          this.closeForm();
      ...
      this.loader.start();
      this.apiService.saveSampleReceipt(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sample Return Saved Successfully');
          this.closeForm();
          this.apiService.applyNotificationCount('Sample');
        }
        if (data['status'] == 0) {
  ...
  showSampleReceipt() {
    this.loader.start();
    this.apiService.showSampleReceipt({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1 && data['data']) {
  ...
  getSampleReceipt() {
    ...
    this.loader.start();
    this.apiService.getSampleReceipt({client_id: this.clientId}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  deleteInfo() {
    if (this.purchaseForm.value.id && this.purchaseForm.value.id > 0) {
      ...
      this.toastr.error('ERROR', 'Please Select Sample Return.');
    }
  }

  deleteData() {
    if (!this.purchaseForm.value.id) {
      this.toastr.error('ERROR', 'Please Select One Sample Return.');
    } else {
      this.loader.start();
      this.apiService.deleteService({id: this.purchaseForm.value.id}).subscribe(data => {
        ...
        if (data && data['status'] == 1) {
          ...
          this.toastr.success('SUCCESS', 'Sample Return deleted successfully.'); 
        }
      });
    }
  }
