...
@Component({
  selector: 'app-sample-request-demand-approval',
  templateUrl: './sample-request-demand-approval.component.html',
  styleUrls: ['./sample-request-demand-approval.component.css']
})
export class SampleRequestDemandApprovalComponent implements OnInit {
  ...
  getSampleDemand(status: string) {
    this.loader.start();
    this.apiService.getSampleDemand({status: status}).subscribe(data => {
      this.loader.stop();
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      ...
      this.apiService.SampleDemandApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sample Demand Approved Successfully.');
          this.closeForm();
