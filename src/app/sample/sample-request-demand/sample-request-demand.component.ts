...
@Component({
  selector: 'app-sample-request-demand',
  templateUrl: './sample-request-demand.component.html',
  styleUrls: ['./sample-request-demand.component.css']
})
export class SampleRequestDemandComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    ...
    this.purchaseForm = this.fb.group({
      ...
      sample_date: new FormControl(null, Validators.required),
      t_sample_date: new FormControl(null),
      sample_time: new FormControl(null, Validators.required),
      sample_days: new FormControl(null, Validators.required),
      sample_return_date: new FormControl(null, Validators.required),
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      $( "#sample_return_date" ).datepicker({ autoclose: true, todayHighlight: true, format: 'dd/mm/yyyy' }).on('changeDate', (e:any) => {
        const date = this.datePipe.transform(e.date, 'yyyy-MM-dd');
        this.purchaseForm.controls.sample_return_date.setValue(date);
      });
      $("#sample_return_date").mask('00/00/0000');
    }, 1000);
  }
  ...
  cancelData(cancel: number) {
    this.apiService.sampleDemandCancel({id: this.purchaseForm.value.id , is_cancel: cancel}).subscribe(data => {
  ...
  getSampleDemand() {
    if (this.clientId) {
      this.apiService.getSampleDemand({client_id: this.clientId}).subscribe(data => {
        if (data && data['status'] == 1) {
  ...
  getEditClients() {
    this.apiService.editClients({page: 'samd'}).subscribe(data => {
      if (data && data['status'] == 1) {
        this.clients = data['data'];
      }
    });
  ...
  getSampleDemandNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getSampleDemandNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.demandNo = data['data'];
          this.purchaseForm.controls.sample_no.setValue(this.demandNo);
        }
        this.loader.stop();
      });
    }
  ...
  setReturnDate() {
    if (this.purchaseForm.value.sample_date && this.purchaseForm.value.sample_days) {
      var date = new Date();
      var sample_date = new Date(this.purchaseForm.value.sample_date);
      date.setDate(sample_date.getDate() + this.purchaseForm.value.sample_days);
      const ddate = this.datePipe.transform(date, 'yyyy-MM-dd');
      this.purchaseForm.controls.sample_return_date.setValue(ddate);
      $("#sample_return_date").datepicker('setDate', date)
    } else {
      this.purchaseForm.controls.sample_return_date.setValue(null);
    }
  }
  ...
  setInitDate() {
    ...
    this.purchaseForm.controls.sample_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.sample_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateSampleDemand(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sample demand details update successfully.');
          this.closeForm();
      ...
      this.loader.start();
      this.apiService.saveSampleDemand(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sample demand details saved successfully.');
          this.closeForm();
          this.apiService.applyNotificationCount('Sample');
          this.productDetails = [];
        }
  ...
  deleteInfo() {
    if (this.purchaseForm.value.id && this.purchaseForm.value.id > 0) {
      ...
      this.toastr.error('ERROR', 'Please Select Sample Demand.');
    }
  }

  deleteData() {
    if (!this.purchaseForm.value.id) {
      this.toastr.error('ERROR', 'Please Select One Sample Demand.');
    } else {
      this.loader.start();
      this.apiService.deleteSampleDemand({id: this.purchaseForm.value.id}).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.closeForm()
          this.showDeleteModal = false
          this.toastr.success('SUCCESS', 'Sample Demand deleted successfully.'); 
        }
      });
    }
  }
  ...
  showData() {
    if (this.demandId) {
      this.loader.start();
      this.apiService.showSampleDemand({id: this.demandId}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.createMode = false;
          this.editMode = true;
