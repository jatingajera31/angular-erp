...
@Component({
  selector: 'app-sample-request-demand-disapproval',
  templateUrl: './sample-request-demand-disapproval.component.html',
  styleUrls: ['./sample-request-demand-disapproval.component.css']
})
export class SampleRequestDemandDisapprovalComponent implements OnInit {
  ...
  getEditClients() {
    this.apiService.editClients({page: 'samd', status: 'Approved', disapprove: 1}).subscribe(data => {
  ...
  getSampleDemand(status: string) {
    this.loader.start();
    this.apiService.getSampleDemand({status: status, client_id: this.clientId, disapprove: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  showSampleDemand() {
    this.loader.start();
    this.apiService.showSampleDemand({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.SampleDemandApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
