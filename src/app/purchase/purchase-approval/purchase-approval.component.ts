...
@Component({
  ...
  styleUrls: ['./purchase-approval.component.css']
})
export class PurchaseApprovalComponent implements OnInit {
  ...
  correctAmount() {
    ...
    this.purchaseForm.controls.invoice_amount.setValue(this.InvoiceAmount);
  ...
  disApprove() {
    if (!this.isNotValid(this.purchaseForm.value.id)) {
      this.loader.start();
      this.apiService.approvedPurchase({ status: 'Pending', id: this.purchaseForm.value.id}).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Purchase Disapproved successfully.');
          this.closeForm();
  ...
  saveAndUpdate() {
    ...
    if (params.id && params.id > 0) {
      ...
      this.apiService.updatePurchase(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Purchase Approved successfully.');
          this.closeForm();
          this.getLatestQrCode();
        }
        if (data['status'] == 0) {
  ...
  getPurchaseOrder() {
    if (this.supplierId) {
      ...
      this.loader.start();
      this.apiService.getPurchase({supplier_id: this.supplierId, status: sts, multi: multi}).subscribe(data => {
        if (data && data['status'] == 1) {
