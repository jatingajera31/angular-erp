...
@Component({
  selector: 'app-project-pre-completion',
  templateUrl: './project-pre-completion.component.html',
  styleUrls: ['./project-pre-completion.component.css']
})
export class ProjectPreCompletionComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    this.purchaseForm = this.fb.group({
      ...
      pre_commissioning_no: new FormControl(null, [Validators.required]),
      pre_date: new FormControl(null, [Validators.required]),
      t_pre_date: new FormControl(null),
      remind_date: new FormControl(null, [Validators.required]),
      t_remind_date: new FormControl(null),
      commissioning_done_by: new FormControl(null, [Validators.pattern("^[0-9]*$")]),
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      ...
      $( "#remind_date" ).datepicker({ autoclose: true, todayHighlight: true, format: 'dd/mm/yyyy' }).on('changeDate', (e:any) => {
        const date = this.datePipe.transform(e.date, 'yyyy-MM-dd');
        this.purchaseForm.controls.remind_date.setValue(date);
      });
      $("#pre_date").mask('00/00/0000');
      $("#remind_date").mask('00/00/0000');
    }, 1000);
  }
  ...
  getPreCommissioningNo() {
    if (!this.isNotValid(this.purchaseForm.value.client_id) && this.createMode) {
      ...
      this.apiService.getPreCommissioningNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.purchaseForm.controls.pre_commissioning_no.setValue(data['data']);
        }
        this.loader.stop();
      });
    }
    ...
    if (this.isNotValid(this.purchaseForm.value.client_id)) {
      this.purchaseForm.controls.pre_commissioning_no.setValue(null);
    }
  }
    ...
    if (!this.isNotValid(this.purchaseForm.value.id)) {
      ...
      this.apiService.showPreCommissioning({id: this.purchaseForm.value.id}).subscribe(data => {
        if (data && data['status'] == 1 && data['data']) {
          ...
          if (data['data']['remind_date']) {
            $("#remind_date").datepicker('setDate', new Date(data['data']['remind_date']))
          }
