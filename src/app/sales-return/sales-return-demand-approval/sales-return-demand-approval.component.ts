...
@Component({
  selector: 'app-sales-return-demand-approval',
  templateUrl: './sales-return-demand-approval.component.html',
  styleUrls: ['./sales-return-demand-approval.component.css']
})
export class SalesReturnDemandApprovalComponent implements OnInit {
  ...
  getSalesReturnDemand(status: string) {
    this.loader.start();
    this.apiService.getSalesReturnDemand({status: status}).subscribe(data => {
      this.loader.stop();
  ...
  viewCreateMode() {
    ...
    this.editMode = false;
    this.purchaseForm.reset();
    this.getSalesReturnDemand('Pending');
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      ...
      this.apiService.salesReturnApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Sales Return Demand Approved Successfully.');
          this.closeForm();
        }
        if (data['status'] == 0) {
