...
@Component({
  selector: 'app-spare-service-demand',
  templateUrl: './spare-service-demand.component.html',
  styleUrls: ['./spare-service-demand.component.css']
})
export class SpareServiceDemandComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    ...
    this.purchaseForm = this.fb.group({
      ...
      spare_no: new FormControl(null, Validators.required),
      ...
      spare_date: new FormControl(null, Validators.required),
      t_spare_date: new FormControl(null),
      spare_time: new FormControl(null, Validators.required),
      spare_days: new FormControl(null, Validators.required),
      spare_return_date: new FormControl(null, Validators.required),
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      $( "#spare_return_date" ).datepicker({ autoclose: true, todayHighlight: true, format: 'dd/mm/yyyy' }).on('changeDate', (e:any) => {
        const date = this.datePipe.transform(e.date, 'yyyy-MM-dd');
        this.purchaseForm.controls.spare_return_date.setValue(date);
      });
      $("#spare_return_date").mask('00/00/0000');
    }, 1000);
  }
  ...
  cancelData(cancel: number) {
    this.apiService.spareDemandCancel({id: this.purchaseForm.value.id , is_cancel: cancel}).subscribe(data => {
  ...
  getSpareDemand() {
    if (this.clientId) {
      this.apiService.getSpareDemand({client_id: this.clientId}).subscribe(data => {
        if (data && data['status'] == 1) {
  ...
  getEditClients() {
    this.apiService.editClients({page: 'spared'}).subscribe(data => {
      if (data && data['status'] == 1) {
        this.clients = data['data'];
      }
    });
  ...
  getSpareDemandNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getSpareDemandNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.demandNo = data['data'];
          this.purchaseForm.controls.spare_no.setValue(this.demandNo);
        }
        this.loader.stop();
      });
    }
  ...
  setReturnDate() {
    if (this.purchaseForm.value.spare_date && this.purchaseForm.value.spare_days) {
      var date = new Date();
      var spare_date = new Date(this.purchaseForm.value.spare_date);
      date.setDate(spare_date.getDate() + this.purchaseForm.value.spare_days);
      const ddate = this.datePipe.transform(date, 'yyyy-MM-dd');
      this.purchaseForm.controls.spare_return_date.setValue(ddate);
      $("#spare_return_date").datepicker('setDate', date)
    } else {
      this.purchaseForm.controls.spare_return_date.setValue(null);
    }
  }
  ...
  setInitDate() {
    ...
    this.purchaseForm.controls.spare_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.spare_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateSpareDemand(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Spare-Service demand details update successfully.');
          this.closeForm();
      ...
      this.loader.start();
      this.apiService.saveSpareDemand(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Spare-Service demand details saved successfully.');
          this.closeForm();
          this.apiService.applyNotificationCount('Spare');
          this.productDetails = [];
        }
  ...
  deleteInfo() {
    if (this.purchaseForm.value.id && this.purchaseForm.value.id > 0) {
      ...
      this.toastr.error('ERROR', 'Please Select Spare Demand.');
    }
  }

  deleteData() {
    if (!this.purchaseForm.value.id) {
      this.toastr.error('ERROR', 'Please Select One Spare Demand.');
    } else {
      this.loader.start();
      this.apiService.deleteSpareDemand({id: this.purchaseForm.value.id}).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.closeForm()
          this.showDeleteModal = false
          this.toastr.success('SUCCESS', 'Spare Demand deleted successfully.'); 
        }
      });
    }
  }
  ...
  showData() {
    if (this.demandId) {
      this.loader.start();
      this.apiService.showSpareDemand({id: this.demandId}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.createMode = false;
          this.editMode = true;
