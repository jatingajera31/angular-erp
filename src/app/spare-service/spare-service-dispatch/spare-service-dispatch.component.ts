...
@Component({
  selector: 'app-spare-service-dispatch',
  templateUrl: './spare-service-dispatch.component.html',
  styleUrls: ['./spare-service-dispatch.component.css']
})
export class SpareServiceDispatchComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private toastr: ToastService, private fb: FormBuilder, private loader: LoaderService, private apiService: ApiService, private route: ActivatedRoute, private router: Router) {
    this.purchaseForm = this.fb.group({
      ...
      spare_id: new FormControl(null, [Validators.required, Validators.pattern("^[0-9]*$")]),
      ...
      spare_dispatch_date: new FormControl(null, [Validators.required]),
      spare_dispatch_time: new FormControl(null, [Validators.required]),
      spare_dispatch_no: new FormControl(null, [Validators.required]),
      t_spare_dispatch_date: new FormControl(null),
  ...
  getEditClients() {
    this.apiService.editClients({page: 'spareddd'}).subscribe(data => {
      if (data && data['status'] == 1) {
  ...
  getSpareDemand(status: string) {
    this.loader.start();
    this.apiService.getSpareDemand({status: status, dispatch: 1}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  getDispatched() {
    ...
    this.loader.start();
    this.apiService.getSpareDispatch({client_id: this.clientId}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  getSpareDispatchNo() {
    if (this.purchaseForm.value.client_id && this.createMode) {
      this.loader.start();
      this.apiService.getSpareDispatchNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
          this.purchaseForm.controls.spare_dispatch_no.setValue(data['data']);
        }
        this.loader.stop();
      });
    }
  ...
  setRow(item: any) {
    this.setInitDate();
    this.purchaseForm.controls.spare_id.setValue(item.id);
  ...
  showSpareDemand() {
    this.loader.start();
    this.apiService.showSpareDemand({id: this.purchaseForm.value.spare_id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  showSpareDispatch() {
    this.loader.start();
    this.apiService.showSpareDispatch({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1 && data['data']) {
        ...
        if (data['spare']) {
          let project = (data['spare'].project) ? data['spare'].project.name: '';
          this.purchaseForm.controls.client.setValue(data['spare'].client.account_name);
          this.purchaseForm.controls.location.setValue(data['spare'].location.name);
          this.purchaseForm.controls.project.setValue(project);
          this.purchaseForm.controls.spare_no.setValue(data['spare'].spare_no);
          this.purchaseForm.controls.spare_date.setValue(data['spare'].spare_date);
          this.purchaseForm.controls.spare_time.setValue(data['spare'].spare_time);
          if (data['spare'].statusby) {
            let n = data['spare'].statusby.first_name +' '+data['spare'].statusby.father_name +' '+data['spare'].statusby.last_name;
            this.purchaseForm.controls.approved_by.setValue(n);
            this.purchaseForm.controls.status_date.setValue(data['spare'].status_date);
            this.purchaseForm.controls.status_time.setValue(data['spare'].status_time);
          }
        }
  ...
  setInitDate() {
    ...
    this.purchaseForm.controls.spare_dispatch_date.setValue(date);
    let tims = this.getTimes(new Date());
    this.purchaseForm.controls.spare_dispatch_time.setValue(tims);
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.updateSpareDispatch(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
      ...
      this.loader.start();
      this.apiService.saveSpareDispatch(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Dispatch Detail Changed Successfully');
          this.closeForm();
