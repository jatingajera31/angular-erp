...
@Component({
  selector: 'app-sales-exchange-demand-approval',
  templateUrl: './sales-exchange-demand-approval.component.html',
  styleUrls: ['./sales-exchange-demand-approval.component.css']
})
export class SalesExchangeDemandApprovalComponent implements OnInit {
  ...
  showExchangeDemand() {
    this.loader.start();
    this.apiService.showExchangeDemand({id: this.purchaseForm.value.id}).subscribe(data => {
      this.loader.stop();
      if (data && data['status'] == 1) {
  ...
  viewCreateMode() {
    ...
    this.purchaseForm.reset();
    this.setDefaultTime();
    this.getExchangeDemand('Pending');
  }
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      this.loader.start();
      this.apiService.ExchangeDemandApproved(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Exchange Demand Approved Successfully.');
          this.closeForm();
        }
        if (data['status'] == 0) {
