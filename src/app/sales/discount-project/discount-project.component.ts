...
@Component({
  selector: 'app-discount-project',
  templateUrl: './discount-project.component.html',
  styleUrls: ['./discount-project.component.css']
})
export class DiscountProjectComponent implements OnInit {
  ...
  getClients() {
    this.apiService.getClients({'project': 1}).subscribe(data => {
  ...
  getEditClients() {
    this.apiService.editClients({page: 'dp'}).subscribe(data => {
      if (data && data['status'] == 1) {
        this.clients = data['data'];
      }
    });
  ...
  getDiscountProjectQuotion() {
    if (!this.isNotValid(this.purchaseForm.value.client_id)) {
      this.getAttentions();
      this.apiService.getDiscountProjectQuotion({client_id: this.purchaseForm.value.client_id, edit_mode: this.editMode}).subscribe(data => {
        if (data && data['status'] == 1) {
  ...
  getDPNo() {
    if (!this.editMode && !this.isNotValid(this.purchaseForm.value.client_id)) {
      this.apiService.getDPNo({client_id: this.purchaseForm.value.client_id}).subscribe(data => {
        if (data && data['status'] == 1) {
    ...
    if (!this.isNotValid(this.purchaseForm.value.quotation_id)) {
      ...
      this.apiService.showQuotation({id: this.purchaseForm.value.quotation_id}).subscribe(data => {
        ...
        if (data && data['status'] == 1) {
          ...
          if (this.isNotValid(data['data']['discount_project_id'])) {
            this.purchaseForm.controls.id.setValue(null);
          } else {
            this.purchaseForm.controls.id.setValue(data['data']['discount_project_id']);
            this.showDP()
          }

          this.DiscountManger = data['data']['discountmg'];
        }
      });
    }
  }
  ...
  showDP() {
    this.loader.start();
    this.apiService.showDP({id: this.purchaseForm.value.id}).subscribe(data => {
      if (data && data['status'] == 1) {
  ...
  saveInfo() {
    ...
    if (params.id && params.id > 0) {
      ...
      if (this.isNotValid(this.purchaseForm.value.discount_approved)) {
        ...
        this.loader.start();
        this.apiService.updateDP(params).subscribe(data => {
          this.loader.stop();
          if (data && data['status'] == 1) {
      ...
      this.loader.start();
      this.apiService.saveDP(params).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.toastr.success('SUCCESS', 'Quotation Discount Approval Successfully.');
          this.closeForm();
  ...
  deleteData() {
    if (!this.purchaseForm.value.id) {
      ...
      this.loader.start();
      this.apiService.deleteDP({id: this.purchaseForm.value.id}).subscribe(data => {
        this.loader.stop();
        if (data && data['status'] == 1) {
          this.closeForm()
